    ...
    <table style="width: 100%;">
        ...
        <tr>
            <td style="text-align: center;">
                <h3>BAGI {{ $ds->format('d-m-Y') }} HINGGA {{ $de->format('d-m-Y') }}</h3>
            </td>
        </tr>
    </table>
{!! Form::open(['method' => 'GET']) !!}
<div class="form-group" align="center">
    <!-- <a target="_blank" href="{!! $uri_gen.'&gen=e' !!}" class="btn btn-success btn-sm"><i class="fa fa-file-excel-o"></i> Muat Turun Excel</a> -->
    <a target="_blank" href="{!! $uri_gen.'&gen=xls' !!}" class="btn btn-success btn-sm">
        <i class="fa fa-file-excel-o"></i> Muat Turun Excel
    </a>
    <a target="_blank" href="{!! $uri_gen.'&gen=p' !!}" class="btn btn-success btn-sm"><i class="fa fa-file-pdf-o"></i> Muat Turun PDF</a>
</div>
{!! Form::close() !!}
    ...
    <table class="table table-striped table-bordered table-hover" style="width: 100%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <!--{{-- $data->BR_BRNNM --}}-->
                    {{ $data->CA_BRNCD ? ($data->BrnCd ? $data->BrnCd->BR_BRNNM : $data->CA_BRNCD) : '' }}
                </td>
                <td>
                    <!--{{-- $cmplcat_list[$data->CA_CMPLCAT] --}}-->
                    {{ $data->CA_CMPLCAT ? ($data->CmplCat ? $data->CmplCat->descr : $data->CA_CMPLCAT) : '' }}
                </td>
                ...
                <td>
                    <!--{{-- $data->CA_INVBY? $data->name:'' --}}-->
                    {{ $data->CA_INVBY ? ($data->InvBy ? $data->InvBy->name : $data->CA_INVBY) : '' }}
                </td>
            </tr>
            @endforeach
        </tbody>
