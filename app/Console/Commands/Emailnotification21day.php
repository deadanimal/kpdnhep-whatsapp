...
use App\Mail\EmelNotifikasiAduanDalamSiasatan21Hari;
use App\Penugasan;
...
class Emailnotification21day extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'eaduan:emailnotification21day';
     ...
     * @var string
     */
    protected $description = 'Send email notification to investigator if complaint is on 21st day';
    ...
    public function handle()
    {
        ...
        foreach ($case_infos as $key => $v) {
            ...
            if($data_final['total_duration'] == 21){
                if($v->namapenyiasat->email){
                    try {
                        // $cc = ['sitizakiah@kpdnhep.gov.my','akmal2012@gmail.com'];
                        ...
                        Mail::to($v->namapenyiasat->email)
                            ...
                            ->send(new EmelNotifikasiAduanDalamSiasatan21Hari($v));
                        $TemplateEmail = 
                            Email::where(['email_type'=>'06','email_code'=>'2','status'=>'1'])
                            ->first();
                        $model = new CaseEmail();
                        $model->ce_title = $TemplateEmail->title;
                        $model->ce_to = $v->namapenyiasat->email;
                        // $model->ce_cc = implode(",",$cc);
                        // $model->ce_cc = $cc;
    ...
    public function query()
    {
        return 
            ...
            ->where('case_info.CA_EMAIL_NOTIFY_21', 0)
            ->get();
    }
}
