...
class FacebookController extends Controller
{
    ...
    {
        $data = FeedFacebook::with('profile')
        ...
        $datatables = DataTables::of($data)
            ->addIndexColumn()
            ->editColumn('profile_id', function (FeedFacebook $data) {
                return $data->profil_id != null ? $data->profile->name : 'Sistem';
            })
            ->editColumn('last_message', function (FeedFacebook $data) {
                return str_replace(PHP_EOL, '_n_', $data->message ?? '');
            })
            ->editColumn('status', function (FeedFacebook $data) {
                if ($data->is_active == 1) {
                    return 'Aktif';
                } else {
                    return 'Tidak Aktif';
            ...
            ->addColumn('action', '
                <a href="{{ route("facebook.show", $id) }}" class="btn btn-xs btn-primary" data-toggle="tooltip" data-placement="right" title="Add to my task">
    ...
    {
        return view('feedback.facebook.index');
    }

    public function mytask()
    {
        return view('feedback.facebook.mytask');
    }

    public function show($id)
    {
        $feeds = FeedFacebook::findOrFail($id);
        $feed_details = $feeds->detail;
        return view('feedback.facebook.show')
            ->with(compact('feeds', 'feed_details'));
    }

    public function edit(Request $request, $id)
    {
        $is_all = $request->get('is_all') ?: 0;
        $feeds = FeedFacebook::where('id', $id)->where('supporter_id', auth()->user()->id)->first();
        if (!$feeds) {
            return $this->index();
        }
        $feed_details = $feeds->detail;
        ...
        return view('feedback.facebook.edit')->with(compact('feeds', 'feed_details', 'is_all'));
    }
