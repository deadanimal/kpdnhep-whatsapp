...
use App\Repositories\Feedback\Email\EmailRepository;
...
class CreateAduanController extends Controller
{
    ...
    {
        $feedback = EmailRepository::findIsMineOrRedirect($id, 'Feedback\Email\EmailController@index');
        ...
        foreach ($feedback_details as $feedback_detail) {
            ...
            if ($feedback_detail['is_have_attachment'] == 1) {
                ...
                $feedback_data['info'] = WhatsappMessageLibrary::separateMessage($feedback_detail['body']);
            }
        ...
        $feedback_data['info']['email'] = $feedback->email;
