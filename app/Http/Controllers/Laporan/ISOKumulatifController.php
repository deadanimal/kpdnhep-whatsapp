...
class ISOKumulatifController extends Controller
{
    ...
    public function perbandingansepuluhkategori(Request $Request)
    {
        ...
        if ($Request->has('cari') || $Request->get('excel') == '1' || $Request->get('pdf') == '1') {
            ...
            */
            ...
            if ($Request->has('CA_RCVDT_MONTH_FROM') && $Request->has('CA_RCVDT_MONTH_TO')) {
                $titlemonth = 'DARI BULAN ' . Ref::GetDescr('206', $Request->get('CA_RCVDT_MONTH_FROM')) . ' HINGGA ' . Ref::GetDescr('206', $Request->get('CA_RCVDT_MONTH_TO'));
                $query->whereMonth('case_info.CA_RCVDT', '>=', $Request->get('CA_RCVDT_MONTH_FROM'));
                $query->whereMonth('case_info.CA_RCVDT', '<=', $Request->get('CA_RCVDT_MONTH_TO'));
            } else {
        ...
        if ($Request->has('excel')) {
            return view('laporan.isokumulatif.perbandingansepuluhkategori.excel', compact('Request', 'datas', 'titleyear', 'titlemonth'));
        } elseif ($Request->has('pdf')) {
            $pdf = PDF::loadView('laporan.isokumulatif.perbandingansepuluhkategori.pdf', compact('Request', 'datas', 'titleyear', 'titlemonth'), [], ['default_font_size' => 7, 'title' => date('Ymd_His'), 'orientation' => 'P', 'format' => 'A4-P']);
            return $pdf->stream('LaporanPerbandingan10Kategori(K)_' . date("Ymd_His") . '.pdf');
        } else {
            return view('laporan.isokumulatif.perbandingansepuluhkategori.index', compact('Request', 'datas', 'titleyear', 'titlemonth'));
        }
    }
    ...
    public function aduannegeridankategori(Request $Request) 
    {
        ...
        */
        ...
        if ($Request->has('excel')) {
            return view('laporan.isokumulatif.aduannegeridankategori.excel', compact('Request', 'datas', 'titleyear', 'titlemonth', 'ListState'));
        } elseif ($Request->has('pdf')) {
            $pdf = PDF::loadView('laporan.isokumulatif.aduannegeridankategori.pdf', compact('Request', 'datas', 'titleyear', 'titlemonth', 'ListState'), [], ['default_font_size' => 7, 'title' => date('Ymd_His'), 'orientation' => 'L', 'format' => 'A4-L']);
            return $pdf->stream('LaporanAduanMengikutNegeriDanKategoriAduan(K)_' . date("Ymd_His") . '.pdf');
        } else {
            return view('laporan.isokumulatif.aduannegeridankategori.index', compact('Request', 'datas', 'titleyear', 'titlemonth', 'ListState'));
        }
    }
    ...
    public function aduanallikutnegeribahagian(Request $Request)
    {
        ...
        if ($Request->has('cari') || $Request->get('excel') == '1' || $Request->get('pdf') == '1') {
            ...
            */
            ...
            if ($Request->has('CA_RCVDT_MONTH_FROM') && $Request->has('CA_RCVDT_MONTH_TO')) {
                $titlemonth = 'DARI BULAN ' . Ref::GetDescr('206', $Request->get('CA_RCVDT_MONTH_FROM')) . ' HINGGA ' . Ref::GetDescr('206', $Request->get('CA_RCVDT_MONTH_TO'));
                $querynegeri->whereMonth('case_info.CA_RCVDT', '>=', $Request->get('CA_RCVDT_MONTH_FROM'));
                $querynegeri->whereMonth('case_info.CA_RCVDT', '<=', $Request->get('CA_RCVDT_MONTH_TO'));
            } else {
                $titlemonth = 'SEMUA BULAN';
            }
            ...
            if ($Request->has('CA_RCVDT_MONTH_FROM') && $Request->has('CA_RCVDT_MONTH_TO')) {
                $queryputrajaya->whereMonth('case_info.CA_RCVDT', '>=', $Request->get('CA_RCVDT_MONTH_FROM'));
                $queryputrajaya->whereMonth('case_info.CA_RCVDT', '<=', $Request->get('CA_RCVDT_MONTH_TO'));
            } else {
                $titlemonth = 'SEMUA BULAN';
            }
        ...
        if ($Request->has('excel')) {
            return view('laporan.isokumulatif.aduanallikutnegeribahagian.excel', compact('Request', 'datas', 'titleyear', 'datap', 'titlemonth'));
        } elseif ($Request->has('pdf')) {
            $pdf = PDF::loadView('laporan.isokumulatif.aduanallikutnegeribahagian.pdf', compact('Request', 'datas', 'datap', 'titleyear', 'titlemonth'), [], ['default_font_size' => 7, 'title' => date('Ymd_His'), 'orientation' => 'P', 'format' => 'A4-P']);
            return $pdf->stream('LaporanAduanSemuaIkutNegeriBahagian(K)_' . date("Ymd_His") . '.pdf');
        } else {
            return view('laporan.isokumulatif.aduanallikutnegeribahagian.index', compact('Request', 'datas', 'datap', 'titleyear', 'titlemonth'));
        }
    }
    ...
    public function aduanallikutnegeribahagian2(Request $Request)
    {
        ...
        if ($Request->has('excel')) {
            return view('laporan.isokumulatif.aduanallikutnegeribahagian2.excel', compact('Request', 'datas', 'titleyear', 'datap', 'titlemonth'));
        } elseif ($Request->has('pdf')) {
            $pdf = PDF::loadView('laporan.isokumulatif.aduanallikutnegeribahagian2.pdf', compact('Request', 'datas', 'datap', 'titleyear', 'titlemonth'), [], ['default_font_size' => 7, 'title' => date('Ymd_His'), 'orientation' => 'P', 'format' => 'A4-P']);
            return $pdf->stream('LaporanAduanSemuaIkutNegeriBahagian(K)_' . date("Ymd_His") . '.pdf');
        } else {
            return view('laporan.isokumulatif.aduanallikutnegeribahagian2.index', compact('Request', 'datas', 'datap', 'titleyear', 'titlemonth'));
        }
    }

    public function aduanallikutnegeribahagian2dd(Request $Request, $year, $month, $state, $brn_cd, $userid, $status)
    {
    ...
    public function aduanallikutnegeribahagianpercent(Request $Request)
    {
        ...
        if ($Request->has('excel')) {
            return view('laporan.isokumulatif.aduanallikutnegeribahagianpercent.excel', compact('Request', 'datas', 'titleyear', 'datap', 'titlemonth'));
        } elseif ($Request->has('pdf')) {
            $pdf = PDF::loadView('laporan.isokumulatif.aduanallikutnegeribahagianpercent.pdf', compact('Request', 'datas', 'datap', 'titleyear', 'titlemonth'), [], ['default_font_size' => 7, 'title' => date('Ymd_His'), 'orientation' => 'P', 'format' => 'A4-P']);
            return $pdf->stream('LaporanAduanSemuaIkutNegeriBahagian(K)_' . date("Ymd_His") . '.pdf');
        } else {
            return view('laporan.isokumulatif.aduanallikutnegeribahagianpercent.index', compact('Request', 'datas', 'datap', 'titleyear', 'titlemonth'));
        }
    }
