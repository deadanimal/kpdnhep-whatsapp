 ...
 * 
 * Laporan Perincian Kelewatan Pengagihan Aduan Mengikut Negeri
 */
...
class Report2Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $states = Ref::where(['cat' => '17', 'status' => '1'])->orderBy('sort', 'asc')
            ->pluck('descr', 'code')->toArray();
        ...
        if ($isSearch) {
            ...
            foreach ($caseInfos as $v) {
                switch (true) {
                    case $v->firstactionreason == 'P1':
                        if(isset($dataCount[$v->BR_STATECD]['P1'])) {
                            $dataCount[$v->BR_STATECD]['P1'] += $v->countcase;
                            $dataCount['total']['P1'] += $v->countcase;
                        }
                        break;
                    case $v->firstactionreason == 'P2':
                        if(isset($dataCount[$v->BR_STATECD]['P2'])) {
                            $dataCount[$v->BR_STATECD]['P2'] += $v->countcase;
                            $dataCount['total']['P2'] += $v->countcase;
                        }
                        break;
                    case $v->firstactionreason == 'P3':
                        if(isset($dataCount[$v->BR_STATECD]['P3'])) {
                            $dataCount[$v->BR_STATECD]['P3'] += $v->countcase;
                            $dataCount['total']['P3'] += $v->countcase;
                        }
                        break;
                    case $v->firstactionreason == 'P4':
                        if(isset($dataCount[$v->BR_STATECD]['P4'])) {
                            $dataCount[$v->BR_STATECD]['P4'] += $v->countcase;
                            $dataCount['total']['P4'] += $v->countcase;
                        }
                        break;
                    default:
                        break;
            ...
            switch ($gen) {
                case 'xls':
                    return view('laporan.ad51.report2.excelxls',
                        compact('request', 'title', 'date_start', 'date_end', 'gen', 'dataCount', 'states', 'caseReasonTemplates', 'countCaseReasonTemplate')
                    );
                    break;
                case 'pdf':
                    $pdf = PDF::loadView('laporan.ad51.report2.pdf',
                        compact('request', 'title', 'date_start', 'date_end', 'gen', 'dataCount', 'states', 'caseReasonTemplates', 'countCaseReasonTemplate')
                    );
                ...
                case 'web':
                default:
                    return view('laporan.ad51.report2.index',
                        compact('request', 'isSearch', 'title', 'date_start', 'date_end', 'gen', 'dataCount', 'states', 'caseReasonTemplates', 'countCaseReasonTemplate')
                    );
                    break;
            }
        }
